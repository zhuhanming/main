...
Currently, Modulo does not allow for the editing of the details of modules, events and deadlines that have been created.
// end::edit[]
...
.. Test case: `delete 1` +
Expected: First module is deleted from the list. All events of that module are deleted.
Details of the deleted module shown in the status message.
.. Test case: `delete 0` +
Expected: No module is deleted. No events deleted.
...
modules' academic year. Another way may be to add random characters that make the JSON format unreadable.
.. Double-click the jar file +

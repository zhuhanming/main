...
This features allow user to set deadline'status to be completed or not completed.The user input is handled and retrieved by the `MainWindow` class in Ui first, after that passed to the `LogicManager` to process the input.
First,`LogicManager` will call the `ModuleParser` which will pass the input to the `DoneCommandParser`.
Then`DoneCommandParser` will invoked `DoneCommand` to set the status of deadline to be true(Completed) and update the model.
Invalid input such as invalid index(i.e done 1000) also handled properly and error message will be displayed to user.
The command is implemented corresponding to the operation in the logic module:

* `DoneCommand` -- Set the status of the deadline to be true.

For the command, a parser is implemented to parse the input into arguments.

* `DoneCommandParser` --Parse the done command input and generates `DoneCommand`.
...
Given below is a activity diagram to show how the how operations work.

image::Done_ActivityDiagram.png[]

==== Design Considerations

===== Aspect: How done features executes

* **Alternative 1 (current choice):** Done 1 deadline at one time
** Pros: Easy to implement,not likely to create bugs.
** Cons: More steps required for user if she/he wants to complete different deadlines.

* **Alternative 2 (current choice):** Done multiple deadlines at one time
** Pros: More time consuming for user to complete list of deadline at one time.
** Cons: User input will be complex as it required to check for different modules and events then finally can set the status to be true.
Difficult to troubleshoot if bugs occur.

* **Alternative 3 :** Modulo will keep track of deadline's status and send reminder to user
** Pros: Remind user which deadline he/she have not completed yet.
** Cons: Hard to implement and it increase coupling.Difficult to write test cases.

* **Future Extension:** Modulo can have another ui panel to display every deadline that needs to be done for every weeks, then user can enter done command to update deadlines' status from there.

...
NOTE: Stateful Logic is implemented by classes that only manages logic while a state exists. This state, within
the AddModuleStatefulLogicManager, is created during the adding of modules by the `module m/MODULE_CODE` function. The
state is depending on the number of event types (tutorial, lecture, lab etc) that each module has. Following
the return of the AddModuleCommandResult, the existence of the AddModuleStatefulLogicManager allows the application
to handle the tutorial, lecture, lab or recitation slot inputs that follows from the `module m/MODULE_CODE` function.

*WRITER* Component:

NOTE: The Writer component is activated when the ExportCommand is executed. It then calls the writeIcsFile function from the IcsWriter class which
retrieves the current list of events from the Model component which, along with its deadlines, are converted into IcsEvent and IcsDeadline

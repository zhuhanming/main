...
* **Alternative 1 (current choice):** Done 1 deadline at a time
** Pros: Easy to implement.
** Cons: More steps required for user to completed different deadlines.
More difficult to implement as it required to check multiple modules and events to set the deadline's status, also more RAM is needed.

* **Alternative 2 :** Modulo will keep track of deadline's status and send reminder to user
** Pros: Remind user which deadline he/she have not completed yet.
** Cons: Hard to implement and it increase coupling.

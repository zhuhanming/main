...
Note how the `Modulo` class implements the `ReadOnlyModulo` interface so that clients who don't need write access to the `Modulo` can access the `Modulo` through the `ReadOnlyModulo` interface instead.
